...
def main() -> int:
    ...
    try:
        ...
        vals_zu_arr = np.asarray(payload["vals_zu"], dtype=float)
        vals_ab_arr = np.asarray(payload["vals_ab"], dtype=int)
        ...
        with pm.Model():
            ...
            trace = pm.sample(
                ...
                tune=int(payload["warmup"]),
                chains=int(payload["chains"]),
                cores=1,
        ...
        for i in range(posterior_draw_count):
            in_sim = rng.lognormal(mean=meanlog_draws[i], sigma=sdlog_draws[i], size=int(payload["n_sim"]))
            prob = alpha_draws[i] / (alpha_draws[i] + mu_draws[i])
            out_sim = rng.negative_binomial(alpha_draws[i], prob, size=int(payload["n_sim"])).astype(float)
            if payload["add_one"]:
                out_sim = out_sim + 1.0

            lrv = np.log10(in_sim / out_sim)
            q10_samples[i] = np.percentile(lrv, int(payload["q"]))

        result = {
            "L_alpha": float(np.percentile(q10_samples, 100 * float(payload["alpha"]))),
            "median": float(np.percentile(q10_samples, 50)),
            "upper_(1-alpha)": float(np.percentile(q10_samples, 100 * (1 - float(payload["alpha"])))),
